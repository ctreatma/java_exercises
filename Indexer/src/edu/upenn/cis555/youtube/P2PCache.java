...
public class P2PCache implements Application {
    ...
    public P2PCache(NodeFactory factory, int daemonPort) throws IOException {
        localCache = new HashMap<String,String>();
        this.factory = factory;
        this.node = this.factory.getNode();
        this.endpoint = this.node.buildEndpoint(this, P2PCache.INSTANCE_NAME);
        ...
        if (daemonPort >= 0) {
            ...
            workers = new ThreadPool(P2PCache.POOL_SIZE, requests, pastryMap, this);
        }
        
        service = new YouTubeService(P2PCache.INSTANCE_NAME);
    ...
    public void route(String mapKey, String keyword) {
        try {
            ...
            YouTubeMessage message = new YouTubeMessage(node.getLocalNodeHandle(), mapKey, YouTubeMessage.Type.QUERY, keyword);
    ...
    public void deliver(Id id, Message message) {
        YouTubeMessage msg = (YouTubeMessage) message;
        ...
        case QUERY:
            ...
            YouTubeMessage response = new YouTubeMessage(node.getLocalNodeHandle(), msg.getMapKey(),
                    YouTubeMessage.Type.RESULT, msg.getKeyword(), responseContents);
    ...
    public static void main(String[] args) {
        try {
            ...
            List<P2PCache> cacheServers = new ArrayList<P2PCache>();
            ...
            cacheServers.add(new P2PCache(factory, daemonPort));
            
            for (int i = 1; i < numNodes; ++i) {
                cacheServers.add(new P2PCache(factory, -1));
            }
        }
        catch (Exception ex) {
